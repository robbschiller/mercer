 ...
 * history, specs, photos, contacts.
 *
 * It renders in two places and they are not the same page: `/properties/[id]`
 * (the building's own record) and `/leads/[id]` (the lead detail view, where
 * this is the context wrapped around one work request). The lead page keys on
 * the lead, so `returnTo` carries the posting page through every embedded form
 * — otherwise saving specs from a lead would bounce you to the property.
 */
...
export function PropertyProfile({
  ...
  photos: Photo[];
  deals: PropertyDeal[];
  /** Page heading — the property on `/properties`, the lead on `/leads`. */
  title: string;
  /** Optional line under the address (the lead page names its building here). */
  subtitle?: string | null;
  backHref: string;
  backLabel: string;
  /** Where the embedded forms post back to. */
  returnTo: string;
  startLeadHref?: string;
  /** Lead workflow band, dropped in under the hero. */
  ...
  const hasOpenDeal = deals.some(
    (d) =>
      (d.kind === "lead" && ["takeoff", "on_hold"].includes(d.status)) ||
    ...
    <>
      ...
      <div className="mb-4 grid grid-cols-1 gap-6 rounded-2xl border bg-card p-5 shadow-[0_1px_2px_rgb(0_0_0/0.04)] md:grid-cols-[176px_minmax(0,1fr)] lg:grid-cols-[176px_minmax(0,1fr)_216px]">
        ...
        <div className="flex min-w-0 flex-col">
          ...
          {subtitle && (
            <p className="mt-1.5 flex items-center gap-1.5 text-[13.5px] font-medium text-foreground/70">
          ...
          >
            {detail.contacts.length === 0 ? (
              <p className="p-4 text-sm text-muted-foreground">
                Nobody linked yet — attach contacts from the panel below so the
                next opportunity knows who decides.
              </p>
            ) : (
...
function RelBandTrack({
  ...
  label: string;
  rows: PropertyRelationshipHistory["management"];
  start: number;
  end: number;
}) {
