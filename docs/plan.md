...
- [ ] **Conversation tab — entity-scoped LLM chat.** A dedicated tab where the user **tags "units"** (Property / Project / Bid / Lead / Account / Contact) into a thread and holds a classic LLM conversation with those records' **live data in memory** — e.g. *"What's the amount of the bid the Disney property accepted, and how far along are their buildings?"* Read-grounded Q&A first; **actions in-chat** (reusing the quick-actions + bid/project mutators, same review boundary as the prompt command bar) later, with UI/chat parity. This is the concrete form of the PRD §5.6 NL surface and an extension of Open-now #1. Tab name TBD (candidates: **Ask** / **Chat** / **Threads** / **Copilot**). Plan: [`docs/build-plans/conversation_tab.plan.md`](build-plans/conversation_tab.plan.md). Needs `ANTHROPIC_API_KEY` (deferred until the business provisions one).
