...
1. [ ] **Stable secondary sort on `getLeads`** — bulk-imported rows share `created_at`, so `LIMIT 100 ORDER BY created_at DESC` returns a non-deterministic slice between requests; property groups visibly shuffle in/out across navigations on the BAAA dataset. Add `id` (or `updated_at`) as a stable tiebreaker. Surfaced while shipping property-grouping (2026-04-26).
