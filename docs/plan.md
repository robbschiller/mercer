...
|----------|---|----------------------|------------|
| Lead capture & qualification | 5.1 | CSV import, mapping, source tags, manual single-lead entry, Places resolve of the CSV property address, lead list/detail, statuses, **property-first Niko table** (one row per property with embedded contacts, account/company, pipeline mix, portfolio count, follow-up rollup, Niko search/filter/sort/pagination), **dedicated routed detail pages** for properties / accounts / contacts (`/leads/properties/[id]`, `/leads/accounts/[id]`, `/leads/contacts/[id]`) with cross-links between accounts ↔ properties ↔ contacts ↔ leads, **account autocomplete** on `/leads/new`, **per-lead outreach state** (`last_contacted_at` / `follow_up_at` / `contact_attempts`) with log-contact + follow-up controls in the detail aside, **additive normalized lead domain model** (`accounts`, `properties`, `contacts`, `property_contacts`, `lead_contacts`, `activity_events`, `audit_log`) with compatibility backfill from flat leads | PRD wants a **qualification agent**: portfolio resolution, public-data pull, paint-timing score, generated brief, confidence-scored ranking (`qualification_score`, `qualification_brief`, `agent_run_id`). Bid-side property capture (satellite, footprint, takeoff) still lives on the bid layer. |
...
1. [~] **Re-root the data model to property-first (M1 critical path).** **Phases 1–2 shipped 2026-05-29.** Phase 1 (property root + owner/management/NTO): migration `015_property_root.sql`, `accounts.type`, `properties.owner_account_id`/`management_account_id`, `property_parties` table, `setPropertyOwnership` + an "Ownership & Notice to Owner" card on the property detail page. Phase 2 (access + archetype + rate config): migrations `016_scope_access.sql` (access_items table + `buildings.archetype`) and `017_rate_config.sql` (rate_config + access_rates jsonb, backfilled from user_defaults); pricing engine now totals an access term; an "Access" scope section on the bid detail page; building archetype select. Decision **Option A** (collapse bids into a property-rooted projects spine) is locked. **Migrations 015–017 applied to the DB 2026-05-29** (verified: `accounts.type`, `properties.owner_account_id`/`management_account_id`, `buildings.archetype`, and the `property_parties`/`access_items`/`rate_config` tables all present). **Phase 3 Stage 1 shipped + applied 2026-05-29:** migration `018_project_spine.sql` folds the delivery `projects` fields onto the bid row (the bid IS the project — `delivery_status`, target/actual dates, assigned_sub, crew_lead, accepted_*, delivery_notes, label), backfilled from the 1:1 projects rows, plus `project_updates.bid_id`. **Stage 2 shipped + verified 2026-05-29:** the data layer is cut over to the bid spine — accept writes `delivery_status`/`accepted_*` on the bid (no separate projects row), project identity = bid id, and every project getter/mutator + `/p/[slug]` status page reads the spine (`project_updates.bid_id`; `project_id` made nullable via `019`). Return shapes held stable via a `ProjectView` mapper, so the pages/actions were untouched; `ownerUserId` scoping preserved. tsc/lint/build clean; DB check: 5 legacy projects == 5 spine projects, 0 mismatches. **Stage 3 shipped + applied 2026-05-29 (destructive):** migration `020_drop_projects.sql` dropped `project_updates.project_id` and the legacy `projects` table (5 rows; data already on the bid spine), set `project_updates.bid_id` NOT NULL, and removed the `projects` table + `Project` type from schema/store. The bid→project collapse is complete. (Cosmetic `bids`→`projects` table rename intentionally skipped — high churn, low value.) Phases 4–5 (proposal snapshot party/access section; docs/cleanup) follow — see [`docs/build-plans/property_rooted_remodel.plan.md`](build-plans/property_rooted_remodel.plan.md). From the 2026-05-29 Jordan working session: the **property** is the top-level object; **projects** are children (one property → many projects — Pura Vita: a blue-section bid and a whole-building bid are two projects on one property, only one will happen); **leads** hang off the property; **contacts** are a parallel object linking to both property and project. The **scope object lives inside a project, not at the root** — surfaces and **access** (lifts / scaffolding / swing stage / safety, scaling by height/archetype not sqft) are sibling dimensions of scope. Owner / management company / contact-role mapping become **first-class, legally distinct** (Notice-to-Owner lien rights; downstream invoice + NTO routing). Surface measure is **paintable square feet**, and the archetype set gains a **seven-story mid-rise** alongside garden/townhome. Do this before more UI — every derived view (proposal, ops, reporting) inherits the object graph. Full reasoning + target shape in *Property-rooted re-model* below. Rob's interviews independently land on property-as-root.
2. [ ] **Specify the prompt-bidding boundary (pre-M2 design).** Jordan wants to bid by prompt ("here are photos, we need swing stage, 150k paintable sf, 40% GP, done") with the AI "already knowing" the rates. Clean split: the **LLM is the front door** that parses the sentence into structured inputs (paintable sf, access method, GP target); a **deterministic engine multiplies those against stored-config rates** to produce the number. "AI knows the rate" = rates are **saved config the engine reads, never numbers the model invents**. Write this boundary down before any prompt-bidding code so a model can't hallucinate a labor rate on a real $300k bid.
3. [~] **Phase G — Onboarding wizard + branded bid surfaces** — **slices 1-2 shipped 2026-05-02**, see Shipped below. Remaining: logo upload + brand fields flowing into `ProposalSnapshot.brand` + PDF / `/p/[slug]` themed render + `/settings` branding card + dismissible banner for existing users. Original scope: replace the silent post-signup redirect with a 3-step wizard at `/onboarding?step=website|confirm|theme`: capture website URL, run an Anthropic Haiku extraction over the homepage HTML (Zod-typed: company name, address, phone, logo URL, primary color), confirm/edit the result, preview the themed bid. Persist to two new tables (`company_profiles` keyed by `user_id` mirroring the `userDefaults` pattern, `onboardings` for per-step timestamps) via `drizzle/manual/012_company_profiles.sql`; logos live in a new public `brand-assets` Supabase Storage bucket. Brand fields flow into bid renders by adding a `brand` block to `ProposalSnapshot` and snapshotting at proposal-generate time (so already-sent proposals do not silently re-theme when the contractor edits later). PDF (`src/lib/pdf/proposal-template.tsx`) and `/p/[slug]` swap the hard-coded "Mercer" header/footer + `#1a1a1a` accents for the snapshotted brand; `/settings` gets a "Bid branding" card with a "Refresh from website" action. **New users:** gated through the wizard via `src/app/(app)/layout.tsx` (no `onboardings` row → redirect). **Existing users:** dismissible "Add your branding" banner on `/dashboard`, no forced redirect. **v1 hard limits:** one theme per user, no per-bid overrides, no font picker, no auto-color-extraction from logo image, no team/multi-tenant brand sharing. Open dependency: PRD §10 Q10 (email confirmation on/off) determines whether the wizard fires immediately on signup or post-confirmation; the `auth/callback` redirect handles either path.
4. [ ] **Follow-up quick chips polish** — outreach state and Niko follow-up filter are shipped, but a one-click "Overdue" / "Due today" chip row could still speed daily pipeline triage.
5. [ ] **Phase A2 — sort leads by estimated bid** — gated on footprint/estimate fields (Phase B1). While B1 is paused, optional: explicit **sort by created date** control if product wants sort without estimates.
6. **Next strategic bet** — demo landed, now the real unlock per PRD §9 is M1 (capture-first bidding) or M3 (lead qualification agent). Both gated on the decisions in *Decisions blocking Milestone 1 / Phase 1* below. Do not start writing agent code until at least the vision-model + evals-platform + ground-truth decisions are made.
...
The current enrichment runner (`src/lib/leads/enrichment-runner.ts`) treats the CSV address as authoritative and only falls back to Places-by-`company` when the CSV doesn't carry one. The list UI now matches the property-first model: `/leads` defaults to a Niko table with one row per property group, embedded contacts, portfolio count, and a property detail side panel. Leads with no address pool into a "No property address" group.

Directions still on the table (PRD §10 Q6 related):

- **[shipped 2026-05-03]** Property-first Niko table on `/leads`. Group key is `resolvedAddress` for legacy rows; rows show embedded contacts, account/company, pipeline mix, portfolio count, earliest follow-up, Niko search/filter/sort/pagination, and a resizable property detail side panel. Contact-row table remains available via `view=contact`. [`src/components/property-leads-table.tsx`](../src/components/property-leads-table.tsx), [`src/components/property-detail-panel.tsx`](../src/components/property-detail-panel.tsx), [`src/app/(app)/leads/page.tsx`](../src/app/(app)/leads/page.tsx).
...
- **[shipped 2026-05-03]** Portfolio count per account/company, surfaced in the property-first table and side panel so Jordan can prioritize the company with 12 properties over the one with 1.
...
### Property-rooted re-model (open) — Jordan working session 2026-05-29

First proper look at Jordan's half of the operation (AQP). It corrects the **root of the object graph**, which sits on the M1 critical path; the rest is validation and polish. Captured here as the authoritative correction (mirrors *Enrichment rethink* above). Full schema deltas, migration sketch, and phased `store.ts` refactor plan: [`docs/build-plans/property_rooted_remodel.plan.md`](build-plans/property_rooted_remodel.plan.md).
...
- **Prompt-bidding boundary** (see *Open now* #2): LLM parses the natural-language ask into structured scope inputs; a deterministic engine multiplies them against **stored-config rates** to produce the number. Rates are config the engine reads, never values the model invents.
...
- **Lead domain model redesign (2026-05-03)** — additive normalized model for property-level opportunities: `accounts`, `properties`, `contacts`, `property_contacts`, `lead_contacts`, `activity_events`, and `audit_log` ([`drizzle/manual/013_lead_domain_model.sql`](../drizzle/manual/013_lead_domain_model.sql), [`src/db/schema.ts`](../src/db/schema.ts), [`docs/lead-data-model.md`](lead-data-model.md)). Existing lead creation and CSV import now normalize rows into durable account/property/contact records, link primary contacts to properties and leads, preserve raw import provenance, write activity events for imports/outreach/status/bid creation, and write structured audit records. Bids now store `property_id` and `primary_contact_id` while retaining `lead_id` during the transition. `/leads` now defaults to a property-first Niko table with embedded contact chips and a resizable property detail side panel; contact-row view remains available as `view=contact`.
- **Lead entity detail pages (2026-05-12)** — promoted the property / account / contact side panels out of the `/leads` table into routed detail pages: `/leads/properties/[id]`, `/leads/accounts/[id]`, `/leads/contacts/[id]` ([`src/app/(app)/leads/properties/[id]/page.tsx`](../src/app/\(app\)/leads/properties/%5Bid%5D/page.tsx), [`src/app/(app)/leads/accounts/[id]/page.tsx`](../src/app/\(app\)/leads/accounts/%5Bid%5D/page.tsx), [`src/app/(app)/leads/contacts/[id]/page.tsx`](../src/app/\(app\)/leads/contacts/%5Bid%5D/page.tsx)), each backed by a new store getter (`getAccountDetail`, `getPropertyDetail`, `getContactDetail`) and a full-page panel component with cross-links between accounts ↔ properties ↔ contacts ↔ leads. Index routes (`/leads/properties`, `/leads/accounts`, `/leads/contacts`) redirect back to the appropriate `/leads?view=…`. Property rows and contact chips in `PropertyLeadsTable` and `LeadsTable` now navigate via Next `<Link>` instead of opening an in-page sidebar — `lead-detail-aside.tsx` was deleted as part of the move. The leads list itself is now a full-bleed container (`/leads/page.tsx` uses `h-[calc(100svh-3.5rem)]`) so the Niko table fills the viewport without the old aside competing for width. `AccountAutocomplete` on `/leads/new` suggests existing accounts so manually-added leads attach to the existing account graph.
