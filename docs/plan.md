...
### Shipped 2026-09-02 — Jordan's top-of-funnel batch (see [`docs/roadmap.md`](roadmap.md))

- [x] **Phase 0 — environments.** New Supabase project `mercer-prod` (`ykbbqwpqyxqmzjxhfmgc`, us-east-1, Pro org) carries the full public schema, the three storage buckets and their policies, auth site URL `https://usemercer.com`, email auto-confirm on. Vercel **production** env → mercer-prod; **preview** env → the old project `mgytgrxpgjhowvvustry`, which is now **staging** (local dev stays on it). Prod redeployed and smoke-tested (sign-up → onboarding). Zero users in prod: Jordan signs up first as org owner, invites Devin from Settings → Members.
- [x] **1a — Property name + job name on every lead.** Visible, required "Name it" band on `/leads/new` (property name seeded from the finder only when it is a real name; job name always typed). `properties.name` NOT NULL (044). Edit form relabeled Job name; property name required there too.
- [x] **1b — Lists + Convert.** `lists` / `list_rows` (045). `/lists`, `/lists/new`, `/lists/[id]`. Import creates a list and nothing else (no contacts/properties/leads); same file + row count is rejected as a re-upload. Per-row **Convert** opens `/leads/new?listRow=…` prefilled; on Add lead the row is stamped `converted_lead_id`. `createLeadsBatch` and `importLeadsAction` removed; `/contacts/import` redirects to `/lists/new`. Lists in sidebar, "+" menu, and Home.
- [x] **1c — Work type as remembered free text.** `WorkTypeField` (chips from `getLeadWorkTypes()` = distinct `unnest(scope_category)` + the old five seeds, plus free text). Work type, rough $, job size, source now editable on the lead. `/reports` "Work by type" panel.
- [x] **Phase 2 — Opportunities as a tracking record** (shipped later the same day). `bids.quote_amount`, `quote_sent_at`, `decision_due_at`, `is_large_job` (046, backfilled from the lead). Header titled by job name (`label`), property second, with a tracking strip (typed quote or engine estimate, sent, decision expected + overdue, size, contact); all editable in the header form. Takeoff / pricing / OSM / AI quote / catalog fold under one `<details>` closed by default (open on `?open=quote` or an in-flight draft; the AI launchpad now redirects with it). `getJobScheduleContext` prefers the bid's own size. Opportunities list shows the typed quote unless the bid is won (contract wins). New-opportunity Client field is the account autocomplete. **Not done:** folding `/pipeline`; a contact picker on the bid (contact shown read-only from `primary_contact_id`).
- [x] **1d — Fields-first lead page.** `PropertyProfile compact`: no aerial tile, facts row, or ledger; property panels folded under "Property — history, specs, photos & contacts". Intake aerial card replaced by a one-line pinned strip.

### Open now (priority order)

0. [~] **Subscription plumbing (Stripe via Vercel Marketplace)** — 2026-09-20, code complete and runtime-verified on staging against the Stripe **sandbox**. Decided model: flat monthly subscription per org, AI included, `ai_usage` stays internal (PRD §4 *Business model and pricing*, §6.4). Shipped: (a) Stripe resource `mercer-stripe` connected under the renobase team, keys on all envs, sandbox Prices + production webhook endpoint created; (b) `subscriptions` table (048, staging only) + `src/lib/billing.ts`; (c) Checkout in subscription mode + customer portal from `/settings/billing`, `/api/stripe/webhook` mirroring status incl. invoice events; (d) `BillingGate` in `(app)/layout.tsx`, inert until `BILLING_ENFORCED=1`, 14-day trial (pre-launch orgs from launch date); (e) usage page reframed to "AI activity"; (f) `ai_usage.actor_user_id` + `cost_usd` frozen at insert. **Remaining to go live:** claim the sandbox into the real Stripe account and recreate Prices + webhook in live mode, apply 048 to prod, confirm price points, put AQP on a plan, flip `BILLING_ENFORCED=1`, decide on Stripe Tax. See worklog 2026-09-20 → Open. Not in scope: metered token billing, per-report credits (design later when EagleView lands).
1. [ ] **Phase 2 follow-ups:** confirm the tracking field set with Jordan (roadmap §6 Q7 — amount / sent / decision / size shipped; add lost reason or a contact picker if he wants them); decide whether `/pipeline` folds into `/opportunities`.
2. [ ] **Phase 1 leftovers:** README route table (`/bids`, `/takeoff-queue` → `/opportunities`, `/pipeline`); "Start lead" on a property should carry the property into the intake; consider back-filling staging's import-sourced leads into lists.
3. [ ] **Dashboard prompt command bar + prompt-bidding boundary (pre-M2 design).** Add a narrow LLM-supported prompt input at the bottom of `/dashboard` so users can initiate basic app actions from natural language without turning Mercer into a generic chatbot. **UI shell shipped 2026-05-30**: fixed bottom-of-viewport dashboard prompt bar with suggested chips, input, submit/loading state, and not-connected result state. Remaining V1 scope: navigation/filtering, basic dashboard answers from existing aggregates, create contact, log contact, set follow-up, and promote a contact/property into a lead/work request behind review; draft project/bid shells stay behind explicit confirmation. Jordan's prompt-bidding ask ("here are photos, we need swing stage, 150k paintable sf, 40% GP, done") stays behind the same boundary: the **LLM is the front door** that parses the sentence into structured inputs (paintable sf, access method, GP target); a **deterministic engine multiplies those against stored-config rates** to produce the number. "AI knows the rate" = rates are **saved config the engine reads, never numbers the model invents**. Implementation plan: [`docs/build-plans/dashboard_prompt_command_bar.plan.md`](build-plans/dashboard_prompt_command_bar.plan.md).
4. [~] **Phase G — Onboarding wizard + branded bid surfaces** — **slices 1-2 shipped 2026-05-02**, see Shipped below. Remaining: logo upload + brand fields flowing into `ProposalSnapshot.brand` + PDF / `/p/[slug]` themed render + `/settings` branding card + dismissible banner for existing users. Original scope: replace the silent post-signup redirect with a 3-step wizard at `/onboarding?step=website|confirm|theme`: capture website URL, run an Anthropic Haiku extraction over the homepage HTML (Zod-typed: company name, address, phone, logo URL, primary color), confirm/edit the result, preview the themed bid. Persist to two new tables (`company_profiles` keyed by `user_id` mirroring the `userDefaults` pattern, `onboardings` for per-step timestamps) via `drizzle/manual/012_company_profiles.sql`; logos live in a new public `brand-assets` Supabase Storage bucket. Brand fields flow into bid renders by adding a `brand` block to `ProposalSnapshot` and snapshotting at proposal-generate time (so already-sent proposals do not silently re-theme when the contractor edits later). PDF (`src/lib/pdf/proposal-template.tsx`) and `/p/[slug]` swap the hard-coded "Mercer" header/footer + `#1a1a1a` accents for the snapshotted brand; `/settings` gets a "Bid branding" card with a "Refresh from website" action. **New users:** gated through the wizard via `src/app/(app)/layout.tsx` (no `onboardings` row → redirect). **Existing users:** dismissible "Add your branding" banner on `/dashboard`, no forced redirect. **v1 hard limits:** one theme per user, no per-bid overrides, no font picker, no auto-color-extraction from logo image, no team/multi-tenant brand sharing. Open dependency: PRD §10 Q10 (email confirmation on/off) determines whether the wizard fires immediately on signup or post-confirmation; the `auth/callback` redirect handles either path.
5. [ ] **Follow-up quick chips polish** — outreach state and Niko follow-up filter are shipped, but a one-click "Overdue" / "Due today" chip row could still speed daily pipeline triage.
6. [ ] **Phase A2 — sort leads by estimated bid** — gated on footprint/estimate fields (Phase B1). While B1 is paused, optional: explicit **sort by created date** control if product wants sort without estimates.
...
- [~] **Conversation tab — entity-scoped LLM chat.** Shipped as the **"Ask"** tab (`/ask`). **P1 shipped 2026-06-09** (read-only, offline mock): tag **units** (Lead / Bid·Project / Property / Contact / Company), ask, get answers grounded in those records' live data — e.g. *"What's the amount of the bid the Disney property accepted, and how far along are their buildings?"* Real search + context resolution run live; only NL generation is mocked until a key exists (then Opus 4.8 takes over, no code change). **Remaining:** P2 persistence (`conversations`/`conversation_messages`), P3 **actions in-chat** (quick-actions + bid/project mutators, same review boundary as the prompt command bar; UI/chat parity), markdown rendering, streaming. The concrete form of the PRD §5.6 NL surface; extends Open-now #1. Plan: [`docs/build-plans/conversation_tab.plan.md`](build-plans/conversation_tab.plan.md). Real answers need `ANTHROPIC_API_KEY` (deferred).
- [ ] **Places-centric capture (Maps-grade place finding).** Make Google Places **central** to record creation and make finding/adding a place **as easy as Google Maps** — a reusable `<PlacePicker>` (autocomplete + map/satellite preview + one-tap confirm) as the front door for `/leads/new`, the dashboard create-lead / start-bid sheets, the bid wizard, and a new first-class **add-property** flow. Captures `placeId`/lat-lng/address into the fields the schema already carries; dedup on `googlePlaceId`. Plan: [`docs/build-plans/places_centric_capture.plan.md`](build-plans/places_centric_capture.plan.md). Gated on the production server-side Places key (see *Decisions needed* below, PRD §10 Q9).
...
- [x] **Business model / app shape** (PRD §10 Q11) — **re-resolved 2026-09-20:** Mercer is a **standalone business**; AQP is the **first paying customer**, live on production. This supersedes the 2026-05-29 "AQP-specific app" framing. Multi-tenancy (already shipped via `org_memberships`), billing, and self-serve onboarding are product requirements. Keep building AQP-deep with clean object seams; AQP's constants stay per-org config. Product name may change to Renobase; undecided, do not rename yet. See PRD §4 *Business model and pricing*.
- [x] **Unit economics target** (PRD §10 Q12) — **resolved 2026-09-20 for shipped features:** model spend is 2 to 4 percent of a $500 to $1000/mo subscription at heavy use, so AI is included in the flat price and the `ai_usage` ledger is internal COGS only. Re-run when M1 vision takeoff ships with real photo counts.
